...
export default function Announcements() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const resp = await fetch(`${BASE}/api/announcements`, {
        headers: headers(),
      });
        ...
        <div className="space-y-4">
          {announcements.map((ann) => {
            ...
            return (
              <div
                key={ann.id}
                className={`glass-card rounded-2xl p-5 md:p-6 border-l-4 ${color.split(" ").pop()} group`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`w-10 h-10 rounded-xl ${color.split(" ").slice(0, 2).join(" ")} flex items-center justify-center shrink-0`}
                  >
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-medium uppercase tracking-wider border ${color}`}
                          >
                            {ann.priority}
                          </span>
...
function AddAnnouncementModal({ open, onClose, onSuccess }: any) {
  ...
  useEffect(() => {
    setForm({ title: "", content: "", priority: "normal", targetRoles: "all" });
    setError("");
  }, [open]);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="e.g. Office will remain closed on..."
          />
        </div>
