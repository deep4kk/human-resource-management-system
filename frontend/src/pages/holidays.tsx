...
export default function Holidays() {
  ...
  const fetchHolidays = async () => {
    setLoading(true);
    try {
      const resp = await fetch(`${BASE}/api/holidays?year=${year}`, {
        headers: headers(),
      });
  ...
  const deleteHoliday = async (id: number) => {
    ...
    await fetch(`${BASE}/api/holidays/${id}`, {
      method: "DELETE",
      headers: headers(),
    });
    fetchHolidays();
  ...
  const typeIcons: Record<string, any> = {
    ...
    national: Star,
    religious: Sun,
    optional: CalendarHeart,
  };
  const typeColors: Record<string, string> = {
    public: "bg-primary/10 text-primary",
    national: "bg-amber-500/10 text-amber-600",
    religious: "bg-purple-500/10 text-purple-600",
    optional: "bg-emerald-500/10 text-emerald-600",
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Company holiday calendar for {year}.
          </p>
        </div>
            ...
            <div key={g.month}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {g.holidays.map((h) => {
                  ...
                  const dayName = new Date(h.date).toLocaleDateString("en-IN", {
                    weekday: "long",
                  });
                  return (
                    <div
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {formatDate(h.date)} &bull; {dayName}
                        </p>
                      </div>
        ...
        {Object.entries(typeColors).map(([type, cls]) => {
          ...
          return (
            <div key={type} className="flex items-center gap-2 text-xs">
              <div
                className={`w-6 h-6 rounded-lg ${cls} flex items-center justify-center`}
              >
...
function AddHolidayModal({ open, onClose, onSuccess }: any) {
  ...
  useEffect(() => {
    setForm({ name: "", date: "", type: "public", isOptional: false });
    setError("");
  }, [open]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const resp = await fetch(`${BASE}/api/holidays`, {
        method: "POST",
        headers: headers(),
