...
export default function Policies() {
  ...
  const deletePolicy = async (id: number) => {
    ...
    await fetch(`${BASE}/api/policies/${id}`, {
      method: "DELETE",
      headers: headers(),
    });
    fetchAll();
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          v{p.version} &bull; Updated {formatDate(p.updatedAt)}
                        </p>
                      </div>
...
function AddPolicyModal({ open, onClose, onSuccess }: any) {
  ...
  useEffect(() => {
    setForm({ title: "", category: "General", content: "", version: "1.0" });
    setError("");
  }, [open]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const resp = await fetch(`${BASE}/api/policies`, {
        method: "POST",
        headers: headers(),
