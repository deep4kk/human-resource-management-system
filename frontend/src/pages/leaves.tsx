...
import { useListLeaves } from "@hrms/api-client";
import {
  Plane,
  CalendarRange,
  Clock,
...
export default function Leaves() {
  ...
  const leaveBalances = (() => {
    ...
    const myLeaves = leaves.filter(
      (l: any) => l.employeeId === user.employeeId && l.status === "approved",
    );
    const annual = myLeaves
      .filter((l: any) => l.leaveType === "annual")
      .reduce((s: number, l: any) => s + l.days, 0);
    const sick = myLeaves
      .filter((l: any) => l.leaveType === "sick")
      .reduce((s: number, l: any) => s + l.days, 0);
    const casual = myLeaves
      .filter((l: any) => l.leaveType === "casual")
      .reduce((s: number, l: any) => s + l.days, 0);
    const unpaid = myLeaves
      .filter((l: any) => l.leaveType === "unpaid")
      .reduce((s: number, l: any) => s + l.days, 0);
    return {
      annual: { total: 20, used: annual },
      sick: { total: 10, used: sick },
      casual: { total: 5, used: casual },
      unpaid: { total: 0, used: unpaid },
    };
  })();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Apply for leaves and track your balances.
          </p>
        </div>
        <button
          ...
          <div key={bal.type} className="glass-card p-5 rounded-2xl">
            ...
            <div className="flex items-end gap-2">
              <span className="text-3xl font-bold text-foreground">
                {bal.total > 0 ? bal.total - bal.used : 0}
              </span>
