...
import { Wallet, Download, CheckCircle2, RefreshCcw, Loader2, Eye } from "lucide-react";
...
function usePayrollData(month: number, year: number) {
  ...
  const fetch_ = async () => {
    setLoading(true);
    try {
      const resp = await fetch(`${BASE}/api/payroll?month=${month}&year=${year}`, {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          {running ? <RefreshCcw className="w-5 h-5 animate-spin" /> : <Wallet className="w-5 h-5" />}
      ...
      {payroll.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="glass-card p-5 rounded-2xl">
            ...
            <p className="text-2xl font-bold text-foreground mt-1">{formatCurrency(totals.basic)}</p>
          </div>
          <div className="glass-card p-5 rounded-2xl">
          ...
          <div className="glass-card p-5 rounded-2xl border-l-4 border-l-primary">
            ...
            <p className="text-2xl font-bold text-primary mt-1">{formatCurrency(totals.net)}</p>
          </div>
        </div>
      )}
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-border/50 flex justify-between items-center bg-secondary/20">
          ...
          <div className="flex gap-2">
            <select value={month} onChange={e => setMonth(Number(e.target.value))} className="h-10 px-3 rounded-lg border border-border bg-background text-sm">
              {months.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
            </select>
            <select value={year} onChange={e => setYear(Number(e.target.value))} className="h-10 px-3 rounded-lg border border-border bg-background text-sm">
                ...
                {payroll.map((record: any) => (
                  <tr key={record.id} className="hover:bg-secondary/10">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => viewPayslip(record.id)} className="text-primary hover:bg-primary/10 p-2 rounded-lg transition-colors inline-flex" title="View Payslip">
                ...
                {payroll.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-muted-foreground">
                      No payroll records for {months[month - 1]} {year}. Click "Run Payroll" to generate.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <Modal open={!!payslip} onClose={() => setPayslip(null)} title="Payslip" maxWidth="max-w-xl">
        {payslip && (
          <div className="space-y-6">
            <div className="text-center border-b border-border/50 pb-4">
              <h3 className="text-xl font-bold text-primary">{payslip.company?.name || "Toyo Kambocha"}</h3>
              <p className="text-sm text-muted-foreground">Payslip for {months[(payslip.payroll?.month || 1) - 1]} {payslip.payroll?.year}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold">{payslip.employee?.designation || "N/A"}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Working Days</p>
                <p className="font-semibold">{payslip.payroll?.presentDays} / {payslip.payroll?.workingDays}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-3">
                <h4 className="font-semibold text-sm text-emerald-600 uppercase tracking-wider">Earnings</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span>Basic Salary</span><span>{formatCurrency(payslip.payroll?.basicSalary)}</span></div>
                  <div className="flex justify-between"><span>HRA</span><span>{formatCurrency(payslip.payroll?.hra)}</span></div>
                  <div className="flex justify-between"><span>Allowances</span><span>{formatCurrency(payslip.payroll?.allowances)}</span></div>
                  <div className="flex justify-between font-bold border-t pt-2"><span>Gross</span><span>{formatCurrency(payslip.payroll?.grossSalary)}</span></div>
                </div>
              </div>
              <div className="space-y-3">
                <h4 className="font-semibold text-sm text-destructive uppercase tracking-wider">Deductions</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span>PF</span><span>{formatCurrency(payslip.payroll?.pf)}</span></div>
                  <div className="flex justify-between"><span>ESI</span><span>{formatCurrency(payslip.payroll?.esi)}</span></div>
                  <div className="flex justify-between"><span>TDS</span><span>{formatCurrency(payslip.payroll?.tds)}</span></div>
                  <div className="flex justify-between font-bold border-t pt-2"><span>Total</span><span>{formatCurrency(payslip.payroll?.deductions)}</span></div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-center">
              ...
              <p className="text-3xl font-bold text-primary">{formatCurrency(payslip.payroll?.netSalary)}</p>
            </div>
          </div>
        )}
      </Modal>
