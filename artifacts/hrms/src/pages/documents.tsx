...
const DEFAULT_TEMPLATES = [
  {
    name: "Offer Letter",
    type: "offer_letter",
    content: `<div style="font-family: 'Segoe UI', sans-serif; max-width: 700px; margin: 0 auto; padding: 40px;">
...
export default function Documents() {
  ...
  const deleteTemplate = async (id: number) => {
    ...
    await fetch(`${BASE}/api/documents/templates/${id}`, {
      method: "DELETE",
      headers: headers(),
    });
    fetchAll();
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Create and manage HR document templates.
          </p>
        </div>
        {isAdmin && (
          <div className="flex gap-2 flex-wrap">
            {templates.length === 0 && (
              <button
                ...
                className="px-4 py-2 bg-accent text-white rounded-xl text-sm font-semibold glass-btn shadow-lg shadow-accent/20 flex items-center gap-2"
              >
            ...
            <button
              ...
              className="px-4 py-2 bg-secondary text-foreground rounded-xl text-sm font-semibold glass-btn border border-border flex items-center gap-2"
            >
      ...
      <div className="flex gap-1 p-1 glass-card rounded-xl w-max">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === "templates" ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:text-foreground"}`}
        >
          Templates ({templates.length})
        </button>
        <button
          onClick={() => setTab("generated")}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === "generated" ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:text-foreground"}`}
        >
            ...
            <div key={t.id} className="glass-card rounded-2xl p-5 space-y-3">
              ...
              <div className="flex flex-wrap gap-1">
                {(t.variables || []).slice(0, 5).map((v: string) => (
                  <span
                    key={v}
                    className="px-2 py-0.5 bg-secondary rounded-md text-[10px] font-medium text-muted-foreground"
                  >{`{{${v}}}`}</span>
                ))}
              ...
              <tbody className="divide-y divide-border/50">
                {generated.map((doc) => (
                  <tr key={doc.id} className="hover:bg-secondary/10">
                    ...
                    <td className="px-4 md:px-6 py-3 hidden sm:table-cell text-muted-foreground">
                      {doc.employeeName}
                    </td>
                    <td className="px-4 md:px-6 py-3 hidden md:table-cell text-muted-foreground">
      ...
      <Modal
        ...
        title={showPreview?.name || "Document"}
        maxWidth="max-w-3xl"
      >
        {showPreview && (
          <div>
            ...
            <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-border/50">
              <button
                onClick={() => {
                  ...
                  if (w) {
                    w.document.write(showPreview.content);
                    w.document.close();
                    w.print();
                  }
                }}
...
function GenerateDocModal({
  ...
  useEffect(() => {
    setForm({ templateId: "", employeeId: "", variables: {} });
    setError("");
  }, [open]);

  const selectedTemplate = templates.find(
    (t: any) => t.id === Number(form.templateId),
  );
  const customVars = (selectedTemplate?.variables || []).filter(
    (v: string) =>
      ![
        "employee_name",
        "first_name",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.templateId || !form.employeeId) {
      setError("Select template and employee.");
      return;
    }
    setSaving(true);
    try {
...
function CreateTemplateModal({ open, onClose, onSuccess }: any) {
  ...
  useEffect(() => {
    setForm({ name: "", type: "custom", content: "", variablesStr: "" });
    setError("");
  }, [open]);
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full h-10 px-3 rounded-lg border border-border bg-background text-sm"
              placeholder="e.g. Appraisal Letter"
            />
          </div>
          <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, variablesStr: e.target.value }))
            }
            className="w-full h-10 px-3 rounded-lg border border-border bg-background text-sm"
            placeholder="hr_name, reporting_manager"
          />
          <p className="text-[10px] text-muted-foreground">
            Auto-filled: employee_name, designation, department, salary,
            join_date, email, date, company_name
          </p>
        </div>
        <div className="space-y-1.5">
