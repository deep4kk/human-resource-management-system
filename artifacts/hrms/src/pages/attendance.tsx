...
export default function Attendance() {
  ...
  const myTodayRecord = today?.records?.find(
    (r: any) => r.employeeId === myEmployeeId,
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Daily attendance tracked via biometric devices.
          </p>
        </div>
        {isAdmin && (
          <a
            href={`${import.meta.env.BASE_URL.replace(/\/$/, "")}/settings/biometrics`}
            className="px-4 py-2 bg-primary text-primary-foreground rounded-xl text-sm font-semibold glass-btn shadow-lg shadow-primary/20 flex items-center gap-2 w-max"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        <div className="md:col-span-2 glass-card p-5 md:p-8 rounded-2xl relative overflow-hidden">
          ...
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <Clock className="text-primary w-5 h-5" /> Today's Status
          </h2>
              ...
              {myTodayRecord && (
                <div className="text-xs text-muted-foreground mt-2 space-y-0.5">
                  ...
                  {myTodayRecord.checkOut && (
                    <p>Check-out: {formatTime(myTodayRecord.checkOut)}</p>
                  )}
                  {myTodayRecord.workHours && (
                    <p>Hours: {myTodayRecord.workHours}h</p>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center p-3 bg-blue-500/10 rounded-lg">
              <div className="flex items-center gap-2 text-blue-600 text-sm">
                <Calendar className="w-4 h-4" /> On Leave
              </div>
            ...
            <tbody className="divide-y divide-border/50">
              {history?.map((record: any) => (
                <tr key={record.id} className="hover:bg-secondary/10">
                  <td className="px-4 md:px-6 py-3 font-medium text-xs md:text-sm">
                    {formatDate(record.date)}
                  </td>
                  <td className="px-4 md:px-6 py-3 text-xs md:text-sm">
                    {record.employeeName}
                  </td>
                  <td className="px-4 md:px-6 py-3 text-muted-foreground hidden sm:table-cell">
                  ...
                  <td className="px-4 md:px-6 py-3">
                    <span
                      className={`capitalize px-2 py-0.5 rounded-full text-[10px] md:text-xs font-medium border ${
