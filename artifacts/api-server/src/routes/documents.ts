...
import { documentTemplatesTable, generatedDocumentsTable, employeesTable, departmentsTable } from "@workspace/db/schema";
...
router.post("/templates", adminOnly, async (req, res) => {
  try {
    ...
    res.status(201).json({ ...template, variables: JSON.parse(template.variables), createdAt: template.createdAt.toISOString(), updatedAt: template.updatedAt.toISOString() });
  } catch (err) {
...
router.put("/templates/:id", adminOnly, async (req, res) => {
  try {
    ...
    res.json({ ...updated, variables: JSON.parse(updated.variables), createdAt: updated.createdAt.toISOString(), updatedAt: updated.updatedAt.toISOString() });
  } catch (err) {
...
router.post("/generate", adminOnly, async (req, res) => {
  try {
    ...
    if (!template[0]) { res.status(404).json({ error: "Template not found" }); return; }

    const emp = await db.select().from(employeesTable).where(eq(employeesTable.id, employeeId)).limit(1);
    if (!emp[0]) { res.status(404).json({ error: "Employee not found" }); return; }

    let deptName = "";
    if (emp[0].departmentId) {
      const dept = await db.select({ name: departmentsTable.name }).from(departmentsTable).where(eq(departmentsTable.id, emp[0].departmentId)).limit(1);
    ...
    const allVars: Record<string, string> = {
      ...
      salary: emp[0].salary ? Number(emp[0].salary).toLocaleString('en-IN') : "",
    ...
    const [doc] = await db.insert(generatedDocumentsTable).values({
      templateId, employeeId, name: docName, content, generatedBy: userId || null,
    }).returning();
...
router.get("/generated", async (req, res) => {
  try {
    ...
    const conditions = employeeId ? eq(generatedDocumentsTable.employeeId, Number(employeeId)) : undefined;
    ...
    .leftJoin(employeesTable, eq(generatedDocumentsTable.employeeId, employeesTable.id))
    .where(conditions)
...
router.get("/generated/:id", async (req, res) => {
  try {
    const doc = await db.select().from(generatedDocumentsTable).where(eq(generatedDocumentsTable.id, Number(req.params.id))).limit(1);
