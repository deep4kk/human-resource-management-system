...
router.post("/", adminOnly, async (req, res) => {
  try {
    ...
    res.status(201).json({ ...ann, createdAt: ann.createdAt.toISOString(), updatedAt: ann.updatedAt.toISOString(), expiresAt: ann.expiresAt?.toISOString() || null });
  } catch (err) {
...
router.put("/:id", adminOnly, async (req, res) => {
  try {
    ...
    const [updated] = await db.update(announcementsTable).set({
      ...
      ...(priority && { priority }), ...(targetRoles && { targetRoles }),
      ...(isActive !== undefined && { isActive }),
      ...(expiresAt !== undefined && { expiresAt: expiresAt ? new Date(expiresAt) : null }),
      updatedAt: new Date(),
    }).where(eq(announcementsTable.id, Number(req.params.id))).returning();
    if (!updated) { res.status(404).json({ error: "Not Found" }); return; }
    res.json({ ...updated, createdAt: updated.createdAt.toISOString(), updatedAt: updated.updatedAt.toISOString(), expiresAt: updated.expiresAt?.toISOString() || null });
  } catch (err) {
