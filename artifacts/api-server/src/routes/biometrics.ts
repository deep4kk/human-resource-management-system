...
router.get("/settings", async (req, res) => {
  try {
    const settings = await db
      .select()
      .from(biometricSettingsTable)
      .orderBy(biometricSettingsTable.createdAt);
    res.json(
      settings.map((s) => ({
        ...s,
...
router.put("/settings/:id", adminOnly, async (req, res) => {
  try {
    ...
    const [updated] = await db
      ...
      .where(eq(biometricSettingsTable.id, id))
      .returning();
    if (!updated) {
  ...
  try {
    ...
    for (const record of records) {
      try {
        ...
        if (!emp[0]) {
          errors.push(`Employee ${employeeCode} not found`);
          skipped++;
          continue;
        }
        ...
        const existing = await db
          ...
          .where(
            and(
              eq(attendanceTable.employeeId, emp[0].id),
